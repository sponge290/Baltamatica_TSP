...
%% ==================== 配置项（仅需修改此处） ====================
config.allow_origin = ["http://localhost:3000", "http://localhost:3001", "https://baltamatica-tsp.vercel.app"]; % 允许的前端域名
...
% 构建ALLOW_ORIGIN的Python列表字符串
allow_origin_str = '[';
for i = 1:length(config.allow_origin)
    allow_origin_str = [allow_origin_str, '"', config.allow_origin(i), '"'];
    if i < length(config.allow_origin)
        allow_origin_str = [allow_origin_str, ', '];
    end
end
allow_origin_str = [allow_origin_str, ']'];

% 重新构建pycode数组，确保所有字符串正确闭合和转义
pycode = {
    'import sys',
    'import json',
    'import time',
    ...
    '',
    '# 配置项',
    'ALLOW_ORIGIN = ' + allow_origin_str,
    'PORT = ' + num2str(config.port),
    'HOST = "' + config.host + '"',
    'MAX_EXEC_TIME = ' + num2str(config.max_exec_time),
    ...
    '                temp_script = "tsp_temp_" + str(int(time.time())) + ".m"',
    '                ',
    '                # 写入临时脚本文件',
    ...
    '                ',
    '                # 调用北太天元执行脚本，捕获输出',
    '                cmd = [BALTAMATICA_PATH, "-batch", "run(\"" + temp_script + "\"")"]',
    '                exec_logs.append("启动" + algorithm + "算法执行，超时时间" + str(MAX_EXEC_TIME) + "秒")',
    '                ',
    ...
    '                self.send_header("Content-Type", "application/json")',
    '                self.end_headers()',
    '                self.wfile.write(json.dumps({"code": 408, "msg": "算法执行超时，超过" + str(MAX_EXEC_TIME) + "秒"}).encode("utf-8"))',
